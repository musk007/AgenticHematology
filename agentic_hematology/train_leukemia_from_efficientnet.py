#!/usr/bin/env python3
"""Train patient-level classifiers from YOLO + EfficientNet pipeline features.

Runs detection (YOLO localizer + trained EfficientNet attribute head) on the LLD
train/test image splits, aggregates per-patient tabular features, then fits Random
Forest, XGBoost, and/or LightGBM on the train split and evaluates on the held-out
test split (same 34/13 patients as wbc_unified det_dataset).
...
Example:
  ...
  python train_leukemia_from_efficientnet.py --backend all \
    --features-cache outputs/ablations/classifier/efficientnet/features.json
"""
...
DEFAULT_EFFNET_ATTR = WBC_CV / "runs" / "attribute" / "train" / "best_attr.pt"
DEFAULT_IMAGE_ROOT = WBC_CV / "generated" / "det_dataset" / "images"
DEFAULT_OUT_DIR = HERE / "outputs" / "ablations" / "classifier" / "efficientnet"
...
def _build_detector(
    *,
    ...
    attr_clf = EfficientNetAttributeClassifier(
        weights_path=str(effnet_weights),
        device=device,
        predicts_cell_type=effnet_predicts_celltype,
    )
    return TwoStageDetectionAgent(
        localizer=localizer,
        attribute_classifier=attr_clf,
        attribute_head_name="EfficientNet",
    )
...
def _rows_to_dataframe(
    ...
    y = pd.Series(y_labels, index=patient_ids, name="diagnosis")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train RF/XGBoost/LightGBM on YOLO + EfficientNet patient features."
    )
    ...
    ap.add_argument("--effnet-weights", type=Path, default=DEFAULT_EFFNET_ATTR)
    ap.add_argument("--effnet-predicts-celltype", action="store_true")
    ...
    if not args.effnet_weights.is_file():
        raise SystemExit(
            f"EfficientNet attribute weights not found: {args.effnet_weights}\n"
            "Train them first with wbc_unified/cv/train_attributes.py"
        )
    ...
    cache_meta = {
        ...
        "effnet_weights": str(args.effnet_weights),
        "effnet_predicts_celltype": args.effnet_predicts_celltype,
    ...
    if args.features_cache and args.features_cache.is_file():
        ...
        print(f"Building detector: YOLO + EfficientNet ({args.effnet_weights.name})")
        detector = _build_detector(
            yolo_weights=args.yolo_weights,
            effnet_weights=args.effnet_weights,
            device=args.device,
            conf_threshold=args.conf_threshold,
            effnet_predicts_celltype=args.effnet_predicts_celltype,
        )
        print(
            f"Extracting features via detection + attribute classification for {len(all_patients)} patients..."
        )
